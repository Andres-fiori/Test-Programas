...
export default class ProductObjectPage extends ControllerExtension<ExtensionAPI> {
	...
	private loadModel(): void {
		...
		let model = new JSONModel(data) as JSONModel;
	...
	public async onSaveSales(): Promise<void> {
		...
		const productId = bindingContext.getObject("ID");
		console.log(model.getData());
		await this.base.getExtensionAPI().getEditFlow().invokeAction("Products.setSales", {
			contexts: bindingContext,
			parameterValues: [
				{ name: "year", value: model.getProperty("/year") },
				{ name: "month", value: model.getProperty("/month") },
				{ name: "quantity", value: model.getProperty("/quantity") }
			],
	...
	public onNavToDetails(): void {
		...
		const bindingContext = extensionAPI.getBindingContext() as Context;

		// Validación crítica para que no se rompa el código
		if (!bindingContext) {
			console.error("No se encontró el contexto de vinculación (bindingContext)");
			return;
		}
		...
		console.log("Valores para navegar:", productId, isActiveEntity, this.base.year, this.base.month);

		// Navegación con el nombre de ruta corregido (sin prefijo)
		extensionAPI.getRouting().navigateToRoute("SalesReviewPage", {
			key: productId,
			boolean: isActiveEntity,
			year: this.base.year || "2024", // Valor por defecto si está vacío
			month: this.base.month || "1"   // Valor por defecto si está vacío
		});
	}
}